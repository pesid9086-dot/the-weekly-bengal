...
function Router() {
  ...
  const logout = useCallback(() => { setSession(null); notify(language === 'bn' ? 'আপনি নিরাপদে বেরিয়ে গেছেন।' : 'You have signed out safely.'); }, [language, notify, setSession]);
...
function Header({ dark, language, setDark, setLanguage, session, logout, t }: { dark: boolean; language: 'bn' | 'en'; setDark: (value: boolean) => void; setLanguage: (value: 'bn' | 'en') => void; session: Session | null; logout: () => void; t: typeof translations.bn }) {
  ...
  return <>
    <header className="wb-header">
      ...
      <div className="wb-ticker"><div className="wb-ticker-track">{[...Array(2)].flatMap(() => ['আজকের নির্বাচিত খবর', 'উপকূলে বই পৌঁছানোর নতুন উদ্যোগ', 'বিশ্বজুড়ে বদলাচ্ছে বাণিজ্যের মানচিত্র', 'শেষ ওভারের আগে: একটি দলের ভেতরের গল্প']).map((item, index) => <div className="wb-ticker-item" key={`${item}-${index}`}><strong>সরাসরি</strong><span>{item}</span><Zap size={12} /></div>)}</div></div>
    </header>
...
function SearchPage({ news, bookmarks, toggleBookmark, t }: { news: News[]; bookmarks: string[]; toggleBookmark: (id: string) => void; t: typeof translations.bn }) {
  ...
  return <main className="wb-container"><div className="wb-page-intro"><div className="wb-kicker">আর্কাইভ / SEARCH</div><h1 className="wb-serif">{t.search}</h1><p className="wb-excerpt">গল্পের ভেতর থেকে গল্প খুঁজে নিন। বিষয়, নাম বা একটি শব্দ দিয়ে শুরু করুন।</p></div><div style={{ paddingTop: '1.3rem' }}><label className="wb-search-bar"><Search size={18} color="hsl(var(--accent))" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="যেমন: নদী, বাজেট, সিনেমা" aria-label="খুঁজুন" data-testid="input-search" /></label><div className="wb-chips"><button className={`wb-chip ${category === 'সব বিভাগ' ? 'active' : ''}`} onClick={() => setCategory('সব বিভাগ')} data-testid="button-filter-all">সব বিভাগ</button>{CATEGORIES.map((item) => <button key={item} className={`wb-chip ${category === item ? 'active' : ''}`} onClick={() => setCategory(item)} data-testid={`button-filter-${item}`}>{item}</button>)}</div></div><div className="wb-section-head"><h2 className="wb-serif">{results.length}টি প্রতিবেদন</h2><p>{query ? `“${query}” অনুসন্ধান` : 'সর্বশেষ প্রকাশিত'}</p></div>{results.length ? <div className="wb-list-grid">{results.map((article) => <StoryCard key={article.id} article={article} bookmarked={bookmarks.includes(article.id)} toggleBookmark={toggleBookmark} />)}</div> : <div className="wb-empty" style={{ margin: '2rem 0 4rem' }}><Search size={30} /><h2 className="wb-serif">কোনো প্রতিবেদন মেলেনি</h2><p>অন্য শব্দ বা বিভাগ দিয়ে আবার চেষ্টা করুন।</p></div>}</main>;
}
...
function Login({ users, setUsers, login, t }: { users: User[]; setUsers: (next: User[] | ((previous: User[]) => User[])) => void; login: (session: Session) => void; t: typeof translations.bn }) {
  ...
  return <main className="wb-auth-wrap"><section className="wb-auth-card wb-reveal"><div className="wb-kicker">THE WEEKLY BENGAL / DESK</div><h1 className="wb-serif" style={{ fontSize: '2.35rem', lineHeight: 1.1, margin: '.5rem 0' }}>{tab === 'signin' ? 'আপনার ডেস্কে প্রবেশ করুন' : 'রিপোর্টার হিসেবে যোগ দিন'}</h1><p style={{ color: 'hsl(var(--muted-foreground))', fontSize: '.8rem', lineHeight: 1.8 }}>সংবাদ পড়া সবার জন্য। নিউজরুমে অবদান রাখতে একটি অ্যাকাউন্ট খুলুন।</p><div className="wb-tabs"><button className={`wb-tab ${tab === 'signin' ? 'active' : ''}`} onClick={() => { setTab('signin'); setError(''); }} data-testid="tab-signin">প্রবেশ</button><button className={`wb-tab ${tab === 'signup' ? 'active' : ''}`} onClick={() => { setTab('signup'); setError(''); }} data-testid="tab-signup">নিবন্ধন</button></div><form className="wb-form-stack" onSubmit={submit}>{tab === 'signup' && <label className="wb-field">আপনার নাম<input value={name} onChange={(event) => setName(event.target.value)} placeholder="নাম লিখুন" autoComplete="name" data-testid="input-name" /></label>}<label className="wb-field">ইমেইল<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" autoComplete="email" data-testid="input-email" /></label><label className="wb-field">পাসওয়ার্ড<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" autoComplete={tab === 'signup' ? 'new-password' : 'current-password'} data-testid="input-password" /></label>{error && <p style={{ color: 'hsl(var(--destructive))', fontSize: '.75rem', margin: 0 }} role="alert" data-testid="status-login-error">{error}</p>}<button className="wb-button wb-button-primary" type="submit" data-testid="button-submit-auth">{tab === 'signin' ? 'প্রবেশ করুন' : 'অ্যাকাউন্ট খুলুন'} <ChevronRight size={15} /></button></form></section></main>;
}
...
function Protected({ session, role, t, children }: { session: Session | null; role: UserRole; t: typeof translations.bn; children: ReactNode }) {
  ...
  if (!session || session.role !== role) return <div className="wb-empty" style={{ margin: '4rem auto', width: 'min(500px, calc(100% - 28px))' }}><ShieldCheck size={34} /><h2 className="wb-serif">অনুমতি প্রয়োজন</h2><p>{t.signIn} করে এই ডেস্কে প্রবেশ করুন।</p></div>;
  return <>{children}</>;
}
...
function Admin({ news, setNews, users, setUsers, notify, syncNews }: { news: News[]; setNews: (next: News[] | ((previous: News[]) => News[])) => void; users: User[]; setUsers: (next: User[] | ((previous: User[]) => User[])) => void; notify: (message: string) => void; syncNews: (article: News | string, operation: 'upsert' | 'delete') => Promise<void> }) {
  ...
  return <main className="wb-container wb-dashboard">
    ...
    <div className="wb-stat-grid"><div className="wb-stat">অপেক্ষমাণ প্রতিবেদন<strong>{pending.length}</strong></div><div className="wb-stat">প্রকাশিত প্রতিবেদন<strong>{news.filter((a) => a.status === 'published').length}</strong></div><div className="wb-stat">সক্রিয় রিপোর্টার<strong>{users.filter((u) => u.role === 'reporter' && u.active !== false).length}</strong></div></div>
